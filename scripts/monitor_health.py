...
def _check_drift(current, baseline, thresholds):
    ...
    if sd is not None:
        if sd > thresholds["max_syllable_deviation"]:
            alerts.append(
                {
                    "metric": "eval_syllable_deviation",
                    "current": sd,
    ...
    if la is not None:
        if la < thresholds["min_line_accuracy"]:
            alerts.append(
                {
                    "metric": "eval_line_count_accuracy",
                    "current": la,
...
def evaluate_model(model_uri):
    ...
    return {
        ...
        "eval_line_count_accuracy": round(sum(line_accs) / len(line_accs), 3)
        if line_accs
        else None,
...
def main():
    ...
    try:
        mlflow.create_experiment(
            "poesia-monitoring", artifact_location="./mlruns/poesia-monitoring"
        )
    except Exception:
        pass
    mlflow.set_experiment("poesia-monitoring")

    print("PoesIA Model Health Monitor")
    print(f"{'=' * 50}")
    ...
    if baseline["eval_syllable_deviation"]["mean"] is not None:
        print(
            f"  Baseline ({args.lookback_days}d, {baseline['eval_syllable_deviation']['count']} runs):"
        )
        print(
            f"    Syllable dev: μ={baseline['eval_syllable_deviation']['mean']:.3f} σ={baseline['eval_syllable_deviation']['std']:.3f}"
        )
        print(
            f"    Line accuracy: μ={baseline['eval_line_count_accuracy']['mean']:.3f} σ={baseline['eval_line_count_accuracy']['std']:.3f}"
        )
    else:
    ...
    print(
        f"  Current: syll_dev={current.get('eval_syllable_deviation', 'N/A')}, "
