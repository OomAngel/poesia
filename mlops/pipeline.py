...
def run_step(description: str, command: list[str]) -> bool:
    ...
    print(f"{'=' * 60}")
    result = subprocess.run(command)  # noqa: S603 - internally-built command list
...
def main():
    ...
    parser.add_argument(
        "--all", action="store_true", help="Run full pipeline: distill → train → evaluate"
    )
    parser.add_argument(
        "--distill-count", type=int, default=100, help="Number of sonetos to distill"
    )
    ...
    if args.all:
        ...
        # Step 1: Distill
        ok = run_step(
            "Distill sonetos from Groq",
            [
                sys.executable,
                "scripts/distill_sonetos.py",
                "--count",
                str(args.distill_count),
            ],
        )
        if not ok:
            sys.exit(1)
    ...
    if args.distill:
        run_step(
            f"Distill {args.distill} sonetos",
            [
                sys.executable,
                "scripts/distill_sonetos.py",
                "--count",
