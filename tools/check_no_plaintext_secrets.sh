#!/usr/bin/env bash
# Reject staged plaintext credentials before they reach the repository.
# Ported from LumiNose (scripts/check_no_plaintext_secrets.sh) and scoped to the
# poesia credential names. Bash implementation only.
set -euo pipefail
...
ALLOWED_VALUES="|replace_me|changeme|example|placeholder|DEMO_KEY|null|None|test-token|dummy|redacted|your_key|your_password|"
