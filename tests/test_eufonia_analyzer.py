...
def test_detect_rhyme_scheme_abab() -> None:
    ...
    scans = [
        ...
        ScanResult(
            line="A dog jumped over the log", rhyme_key=RhymeKey(consonant="og", assonant="o")
        ),
        ScanResult(line="He wore a funny hat", rhyme_key=RhymeKey(consonant="at", assonant="a")),
        ScanResult(
            line="And walked across the fog", rhyme_key=RhymeKey(consonant="og", assonant="o")
        ),
    ]
...
def test_euphony_analysis_scores() -> None:
    ...
    scans = [
        ScanResult(
            line="Solo la soledad de las olas", rhyme_key=RhymeKey(consonant="as", assonant="oa")
        ),
